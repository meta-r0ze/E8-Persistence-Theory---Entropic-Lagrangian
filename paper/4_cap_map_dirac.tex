...
\begin{enumerate}
    ...
    \item \textit{Mass:} A local rotation mixing chiral components with topological impedance (mass) $m$, mediated by the temporal projection matrix $\gamma^0$. (As we will rigorously prove in \cref{sec:capacity_mechanism}, this impedance $m$ cannot be a rigid bare constant due to orthogonal sub-lattices, but must dynamically emerge from the Governor field).
\end{enumerate}
...
\textbf{Lattice Constraint:} To preserve the Lorentzian causal structure derived from the $E_8 \to D_4 \oplus D_4$ projection \cite{meyer_informational_2026}, spatial transitions to nearest-neighbor nodes ($e_k$) must be mediated by internal geometric generators. These transition matrices ($\Gamma_\mu$) must satisfy the metric of the causal manifold, strictly enforcing the anti-commutation relation for the $D_4$ causal lattice basis:
\begin{equation}
    \{\Gamma_\mu, \Gamma_\nu\} \equiv \Gamma_\mu\Gamma_\nu + \Gamma_\nu\Gamma_\mu = 2\eta_{\mu\nu}\mathbb{I} \label{eq:anti_commute}
\end{equation}
where $\eta_{\mu\nu} = \text{diag}(-1, +1, +1, +1)$ is the emergent Lorentzian metric signature.

\textbf{Recognition:} Equation \ref{eq:anti_commute} is exactly the defining algebra of the Dirac gamma matrices ($\gamma^\mu$). Therefore, we explicitly identify the internal lattice transition matrices with the gamma matrices ($\Gamma^\mu \equiv \gamma^\mu$).
...
\begin{equation}
    (i\gamma^\mu \partial_\mu - m)\psi = 0 \label{eq:covariant_dirac}
\end{equation}
