...
\begin{enumerate}
    \item \textit{Shift:} Information propagates to nearest neighbors with geometric admittance (hopping amplitude) $w$ at speed $c = \ell/\tau = 1$, mediated by internal spatial transition matrices $\Gamma^k$.
    \item \textit{Coin (Mass):} A local rotation mixing chiral components with topological impedance (mass) $m$, mediated by the temporal projection matrix $\gamma^0$.
\end{enumerate}
...
\begin{equation}
\begin{aligned}
    \frac{\psi(x,t+\tau) - \psi(x,t)}{\tau} \approx \\
    -ig \sum_{k=1}^3 \Gamma^k \left( \frac{\psi(x+e_k\ell,t) - \psi(x-e_k\ell,t)}{2\ell} \right) &\frac{2\ell}{\tau} - i\gamma^0 m\psi(x,t)
\end{aligned}
\label{eq:diff_form}
\end{equation}
...
\textbf{The Geometric Constraint.} Standard QFT treats $g$ as a tunable coupling. Here, the kinematic bandwidth limit $c \equiv \ell/\tau = 1$ strictly enforces $g = 1/2$. To see this, note that the central difference quotient spans $2\ell$ (from $-\ell$ to $+\ell$), so matching to the continuous derivative $\partial_k\psi$ requires the coefficient $-iw(2\ell/\tau) = -i(2gc)$ to become $-i$ (the unit Dirac velocity). Thus $2w = 1$, fixing $w=1/2$ with zero adjustable parameters.
...
The critical step in identifying this lattice rule with the Dirac equation is the geometric nature of the propagation matrices $\Gamma^k$. Standard scalar cellular automata do not generate anti-commuting matrices. However, as established by the Protocol pillar, the local node state is not a scalar; it requires a complex, chiral bit-depth ($\nu=16$) to encode phase and distinguish input from output. 

Consequently, moving one spatial step ($e_k$) requires applying a corresponding geometric generator to the internal spinor state. These internal matrices must satisfy the metric of the causal manifold. The anti-commutation relation for the $D_4$ causal lattice basis is:
\begin{equation}
    \{\Gamma_\mu, \Gamma_\nu\} \equiv \Gamma_\mu \Gamma_\nu + \Gamma_\nu \Gamma_\mu = 2\eta_{\mu\nu} \mathbb{I}
\end{equation}
where $\eta_{\mu\nu} = \text{diag}(-1,+1,+1,+1)$ is the Lorentzian metric signature derived from the causal projection.

This relation is precisely the defining algebra of the Dirac gamma matrices ($\gamma^\mu$). Therefore, we explicitly identify the internal lattice transition matrices with the gamma matrices ($\Gamma^\mu \equiv \gamma^\mu$).

To recover the covariant Dirac equation, we recognize that the spatial QLGA matrices correspond to the Dirac $\alpha$ matrices ($\Gamma^k = \alpha^k = \gamma^0 \gamma^k$). Multiplying the entire continuum equation by $\gamma^0$ (and utilizing the Lorentzian metric property $(\gamma^0)^2 = -1$) perfectly recovers the covariant Dirac equation:
\begin{equation}
    (i\gamma^\mu \partial_\mu - m)\psi = 0
\end{equation}

\subsection{The Chiral Filter: Geometric Origin of Parity Violation}
Standard QFT inserts the chiral projection $P_L = \frac{1-\gamma^5}{2}$ by hand to match the observed maximal parity violation of the weak interaction. Here, this asymmetry is not postulated; it emerges structurally from the substrate geometry.
...
Consequently, the Dirac equation derived above automatically decomposes. The left-handed components evolve actively under the temporal generator, while the right-handed components remain static with respect to the internal symmetry's active temporal flow. 
