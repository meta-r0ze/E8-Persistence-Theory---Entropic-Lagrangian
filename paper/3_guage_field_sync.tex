...
Having derived the propagator for the node state (Map), we must now derive the dynamics of the links connecting them. In the Universal Architecture, the \textbf{Protocol} pillar requires the system to maintain coherence across spatially separated nodes.

In a discrete lattice, this manifests as the requirement for \textbf{Local Phase Invariance}. If the internal phase of a node rotates, the information flow to its neighbors must rotate compensatorily to preserve the signal. The "Gauge Field" emerges not as a fundamental entity, but as the \textbf{Correction Signal} required to keep the lattice synchronized.
